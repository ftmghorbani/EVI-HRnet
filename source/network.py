...
from source.modules import UpSampleModule, DownSampleModule, AttentionDownSampleModule, BaseModule
...
class CoarseNet(nn.Module):
    ...
    def __init__(self, nc_in, nc_out, nf, use_bias, norm, conv_by, conv_type, nc_ref, use_skip_connection: bool = False):
        super().__init__()
        self.conv_type = conv_type
        self.downsample_module = DownSampleModule(
            nc_in, nf, use_bias, norm, conv_by, conv_type, nc_ref)
    ...
    # Ensure that inpainted and masked_imgs have the same size
        if masked_imgs.size() != inpainted.size():
        # resize masked_imgs to match the size of inpainted
            masked_imgs = masked_imgs[:, :, :inpainted.size(2), :inpainted.size(3)]
...
class Generator(nn.Module):
    def __init__(
        ...
        use_skip_connection: bool = False, nc_ref=3,
    ):
        super().__init__()
        self.nc_ref = nc_ref
        self.coarse_net = CoarseNet(
            nc_in + nc_ref, nc_out, nf, use_bias, norm, conv_by, conv_type, use_skip_connection
        )
    ...
    def forward(self, masked_imgs, ref_frames, masks, guidances=None):
        input_data = torch.cat((masked_imgs, ref_frames, masks), dim=2)
        coarse_outputs = self.coarse_net(input_data, masks, guidances)
        if self.use_refine:
